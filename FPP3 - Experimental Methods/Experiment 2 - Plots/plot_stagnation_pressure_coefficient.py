""" Created on Sun Oct 31 2021, Author: Robert Sales, File: plot_stagnation_pressure_coefficient.py """
...
def StgPressCoeff(p_l, p_r, p_c, p_0):
...
def PlotStagnationPressureCoefficient(save_fig = False):
        ...
        for i, setting in enumerate(["low", "med", "high"]):
            ...
            stg_press_coeff = np.zeros(angl_readings.shape)
            ...
            s_x_2 = t_multiplier * ((3.7659)/(n_readings**0.5)) # p_l
            s_x_3 = t_multiplier * ((3.7659)/(n_readings**0.5)) # p_c
            s_x_4 = t_multiplier * ((3.7659)/(n_readings**0.5)) # p_r
            ...
            for row in range(0,len(angl_readings)):
                ...
                stg_press_coeff[row] = StgPressCoeff(p_l, p_r, p_c, p_0)
                
                # Calculate and append the measurement errors
                b_cp0_1 = ((1)/(p_c - 0.5*(p_l + p_r)))**2 * (p_bias_1**2)
                b_cp0_2 = ((0.5*(p_0 - p_c))/((p_c - 0.5*(p_l + p_r))**2))**2 * (p_bias_2**2)
                b_cp0_3 = ((0.5*(p_l + p_r) - p_0)/((p_c - 0.5*(p_l + p_r))**2))**2 * (p_bias_3**2)
                b_cp0_4 = ((0.5*(p_0 - p_c))/((p_c - 0.5*(p_l + p_r))**2))**2 * (p_bias_4**2)
                
                # Calculate and append the precision errors
                s_cp0_1 = ((1)/(p_c - 0.5*(p_l + p_r)))**2 * (s_x_1**2)
                s_cp0_2 = ((0.5*(p_0 - p_c))/((p_c - 0.5*(p_l + p_r))**2))**2 * (s_x_2**2)
                s_cp0_3 = ((0.5*(p_l + p_r) - p_0)/((p_c - 0.5*(p_l + p_r))**2))**2 * (s_x_3**2)
                s_cp0_4 = ((0.5*(p_0 - p_c))/((p_c - 0.5*(p_l + p_r))**2))**2 * (s_x_4**2)
                
                b_cp0 = (b_cp0_1 + b_cp0_2 + b_cp0_3 + b_cp0_4)**0.5
                s_cp0 = (s_cp0_1 + s_cp0_2 + s_cp0_3 + s_cp0_4)**0.5
                u_cp0 = (b_cp0**2 + s_cp0**2)**0.5
            
                y_p_error[row] = (stg_press_coeff[row] + u_cp0)
                y_m_error[row] = (stg_press_coeff[row] - u_cp0)
        ...
        plt.title(r"Stagnation Pressure Coefficient vs. Relative Flow Angle")
        ...
        if save_fig: plt.savefig('exp_2_stg_coefficient.svg')
        plt.show() 
#==============================================================================

#==============================================================================

def SaveStagnationPressureCoefficient(setting = "low"):
        ...
        for setting in ["high", "med", "low"]:
            ...
            data = np.concatenate((angl_readings,stg_press_coeff), axis=1)
            np.savetxt("stagnation_pressure_coeff_Re_{}.txt".format(setting), data, delimiter='\t', fmt='%10.5f', newline='\n', encoding=None)

#==============================================================================

PlotStagnationPressureCoefficient(save_fig = False)
#SaveStagnationPressureCoefficient()